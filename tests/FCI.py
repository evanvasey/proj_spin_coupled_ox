...
def run_fci_over_r_array_and_save(r_array, mol_func, ncas,nelecas,savetxt):
    ...
    for r in r_array:
        ...
        h1e = mc.get_h1eff()
        eri = mc.get_veff(mc.mo_coeff)
        norb = ncas
        nelec = nelecas

        # Run FCI calculation
        fci_solver = fci.FCI(mc)
        e_fci, ci_fci = fci_solver.kernel(h1e,eri,norb,nelec)
...
fci_energies,mcscf_energies = run_fci_over_r_array_and_save(r_array, H2_H_mol, savetxt="H2H_FCI")
plt.plot(r_array,fci_energies)
plt.plot(r_array,mcscf_energies)
plt.show()
