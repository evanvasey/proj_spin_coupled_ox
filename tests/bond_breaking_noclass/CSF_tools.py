...
def permute_vector(vector,permutation):
    ...
    # permute the vector
    for i in range(n_dim):
        perm_index = permutation[i]
        new_vector[perm_index] = initial_vector[i]

    # calculate the phase factor associated with the permutation
    pf = 1 
    occ_indices = [index for index, value in enumerate(vector) if round(value) == 1]
    perm_indices = []
    for i in occ_indices:
        perm_index = permutation[i]
        perm_indices.append(perm_index)
    occ_dim = len(occ_indices)
    for i in range(occ_dim):
        perm_index = perm_indices[i]
        right_indices = perm_indices[i:]
        for j in right_indices:
            if perm_index > j:
                pf *= -1 
    return new_vector,pf
